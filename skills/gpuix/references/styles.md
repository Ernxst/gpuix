...
- **A bare `hoverWithin` with no `hoverGroup` ancestor never applies, and nothing reports it.** Put `hoverGroup` on the ancestor. A hand-written `hoverWithinGroup` that names a group no ancestor declares is reported in strict mode: `no ancestor hoverGroup named "x" was found`. A CSS-module-generated binding with no matching selector ancestor never applies and stays silent, matching CSS.
