...
{
  "devDependencies": {
    "@gpuix/plugins": "file:/absolute/path/gpuix-plugins-0.20.0-fork.1.tgz",
    "vite": "^8.2.1"
  }
}
```
...
Enter and Shift+Enter insert a newline in a `<textarea>`, and Enter inserts nothing in an `<input>`; both deliver `onKeyDown` with `key: "Enter"` first, and `preventDefault()` there cancels the newline, exactly as in react-dom. Navigation and deletion keys deliver `onKeyDown` before their caret action, and `preventDefault()` cancels that action. Keys typed while that `onKeyDown` is still deciding are applied afterward, in the order they arrived, so a fast typist can never overtake their own Enter or editor action — mouse input is never held behind it. A composer that submits on Enter is written the react-dom way:
...
top of a file, is enough. This holds regardless of `isolate` — `expect.extend`
registers once for the whole worker either way. The window-close and
