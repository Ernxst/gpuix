...
const nativeTestRendererAvailable = isNativeTestRendererAvailable()
const describeNative = nativeTestRendererAvailable ? describe : describe.skip
...
const { width: WIDTH, height: HEIGHT } = nativeTestRendererAvailable
...
describeNative('timeline example', () => {
  ...
  it('drags correctly when the app measures the window itself', async () => {
    ...
    const windowSize = root.renderer.getWindowSize()
    expect(windowSize).toMatchObject({ width: WIDTH, height: HEIGHT })
    expect(windowSize.scaleFactor).toBeGreaterThan(0)
