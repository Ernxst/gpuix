...
impl StyleTable {
    fn find(&self, raw: &[u8]) -> (u64, Option<Arc<StyleDesc>>) {
        let mut hasher = rustc_hash::FxHasher::default();
        raw.hash(&mut hasher);
        let key = hasher.finish();
        let hit = self.entries.get(&key).and_then(|bucket| {
            bucket
                .iter()
    ...
    fn insert(&mut self, key: u64, raw: &[u8], style: StyleDesc) -> Arc<StyleDesc> {
    ...
    pub fn intern(&mut self, raw: &[u8]) -> Result<Arc<StyleDesc>, String> {
        let (key, hit) = self.find(raw);
        if let Some(hit) = hit {
            return Ok(hit);
        }
        let style: StyleDesc = serde_json::from_slice(raw).map_err(|error| error.to_string())?;
        Ok(self.insert(key, raw, style))
    }

    /// Store a style that has already gone through the diagnostic parser.
    /// This preserves content interning without parsing the same payload again.
    pub fn intern_parsed(&mut self, raw: &[u8], style: StyleDesc) -> Arc<StyleDesc> {
        let (key, hit) = self.find(raw);
        hit.unwrap_or_else(|| self.insert(key, raw, style))
    }
