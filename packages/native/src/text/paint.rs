...
pub enum HighlightSource {
    ...
    /// A nested `<text>` tree flattened into one GPUI text layout. Each tuple
    /// maps a retained host's local match ranges into the flattened string.
    FlattenedResolved(
        Arc<super::search::HighlightContext>,
        Vec<(Range<usize>, u64)>,
    ),
...
impl RunStyledText {
    fn new(
        ...
        Self {
            styled: apply_styled_text_accessibility(StyledText::new(text.clone()), accessibility),
            text,
            runs,
        }
    }
    ...
    let (layout, styled) = match run_styles {
        Some(run_styles) => match validate_runs(&text, &run_styles) {
            ...
            Err(error) => {
                ...
                let styled =
                    apply_styled_text_accessibility(StyledText::new(text.clone()), &accessibility);
                (styled.layout().clone(), styled.into_any_element())
            }
        },
        None => {
    ...
    let underlay = canvas(
        ...
        move |_, _, window, _| {
            ...
            let washes = match &highlight {
                ...
                Some(HighlightSource::FlattenedResolved(ctx, mappings)) => {
                    super::search::washes_for_flattened_retained_run(ctx, mappings)
                }
