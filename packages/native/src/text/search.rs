...
/// Washes for retained hosts that the inline-text layer paints as one layout.
/// `mappings` are flattened byte ranges paired with their retained element id;
/// each stored match range is local to that element's own painted content.
pub fn washes_for_flattened_retained_run(
    ctx: &HighlightContext,
    mappings: &[(Range<usize>, u64)],
) -> Vec<Wash> {
    let mut mappings = mappings.to_vec();
    mappings.sort_by_key(|(range, _)| (range.start, range.end));
    let mut out = Vec::new();
    for (flattened, element_id) in mappings {
        let key = crate::text::selection_key(element_id, 0);
        let Some(entries) = ctx.matches.by_key.get(&key) else {
            continue;
        };
        for entry in entries {
            let Some(spec) = ctx.set.specs.get(entry.spec) else {
                continue;
            };
            let range = (flattened.start + entry.range.start)..(flattened.start + entry.range.end);
            if range.end <= flattened.end {
                out.push(wash(
                    ctx,
                    entry.spec,
                    spec,
                    range,
                    MatchId::Retained(entry.index),
                ));
            }
        }
    }
