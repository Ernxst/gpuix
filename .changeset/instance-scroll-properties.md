...
Reading a scroll property forces layout, as `Element.scrollHeight` does, so the chat-autoscroll idiom `el.scrollTop = el.scrollHeight` sees the real extent from a mount effect. `scrollIntoView()` defaults to the DOM's `block: "start"` and honors `block: "nearest"`; alignments gpui cannot express throw instead of scrolling the wrong distance. The four extents round to whole pixels, matching their `long` DOM types.
